...
const TiktokChat = () => {
  ...
  const isSetup = useSelector(state => state.setup.tiktok.isSetup)
  const chatMsgs = useSelector(state => state.tiktok.messages)
  const chatUsers = useSelector(state => state.tiktok.users)
  const order = useSelector(state => state.core.order)

  const handleMoveLeft = () => {
    const arr = reorderLeft(order, TIKTOK_TYPE)
    dispatch(updateOrder({ order: arr }))
  }

  const handleMoveRight = () => {
    const arr = reorderRight(order, TIKTOK_TYPE)
    dispatch(updateOrder({ order: arr }))
  }
  ...
  return (
    <ChatDisplay
      icon="tiktok"
      userCount={chatUsers}
      orderIdx={order.indexOf(TIKTOK_TYPE)}
